...
class PriorityQueue:
    ...
    def __init__(self, maxsize: int = 0):
        self.queues = {
            priority: queue.Queue(maxsize=maxsize)
...
class ProviderRegistry:
    ...
    def get_healthy_providers(self) -> List[BaseProvider]:
        ...
        return [
            provider for provider in self.provider_instances.values()
...
class LoadBalancer:
    ...
    def select_provider(self) -> Optional[BaseProvider]:
        ...
        healthy_providers = [p for p in self.providers if p.get_status() != ProviderStatus.UNHEALTHY]
