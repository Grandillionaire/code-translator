...
class AdvancedTranslatorEngine:
    ...
    def __init__(
        ...
        self.load_balancer = LoadBalancer(
            self.provider_registry.get_healthy_providers(), strategy="best_performance"
        )
    ...
    async def translate(
        ...
        try:
            ...
            result = await self.provider_chain.execute(
                "translate_code", code, source_lang, target_lang, **kwargs
            )
...
class EnhancedTranslatorWindow(TranslatorWindow):
    ...
    def _check_health(self):
        ...
        if "providers" in health:
            healthy_providers = [
                name for name, info in health["providers"].items() if info["status"] == "healthy"
            ]
    ...
    async def translate_code_async(self, code: str, source_lang: str, target_lang: str):
        ...
        try:
            ...
            QMessageBox.warning(self, "Translation Error", error_info.user_message)

    def closeEvent(self, event):
        """Handle application close with proper cleanup"""
        # Save configuration
        self.config_manager.export(
            self.config_manager.config_dir / "config_backup.json", include_sensitive=True
        )
...
async def main_async():
    ...
    lifecycle_manager = ApplicationLifecycleManager(
        app_name="CodeTranslator", enable_monitoring=True
    )
    ...
    translator_engine = AdvancedTranslatorEngine(config_manager, provider_registry, error_handler)
    ...
    window = EnhancedTranslatorWindow(
        config_manager, translator_engine, error_handler, lifecycle_manager
    )
