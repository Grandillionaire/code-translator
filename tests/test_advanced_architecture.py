...
class TestConfigurationManager:
    ...
    def test_multi_backend_support(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Test JSON backend
            json_manager = ConfigurationManager(config_dir / "json", backend=StorageBackend.JSON)
            json_manager.set("format", "json")

            # Test YAML backend
            yaml_manager = ConfigurationManager(config_dir / "yaml", backend=StorageBackend.YAML)
            yaml_manager.set("format", "yaml")

            # Test SQLite backend
            sqlite_manager = ConfigurationManager(
                config_dir / "sqlite", backend=StorageBackend.SQLITE
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                info = handler.handle_error(
                    e, component="test_component", operation="test_operation", user_id="test_user"
                )
...
class TestLifecycleManagement:
    ...
    async def test_initialization_sequence(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            manager = ApplicationLifecycleManager(
                config_dir=Path(tmpdir), enable_monitoring=False  # Disable for testing
            )
    ...
    async def test_graceful_shutdown(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            manager = ApplicationLifecycleManager(config_dir=Path(tmpdir), enable_monitoring=False)
...
async def test_full_integration():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        lifecycle = ApplicationLifecycleManager(
            app_name="TestApp", config_dir=config_dir, enable_monitoring=True
        )
